...
# The simplest and most common type of nucleic acid mutation is a point mutation, which replaces one base with
...
# common ancestor, we get a measure of the minimum number of point mutations that could have occurred on the
...
class DNA
  ...
  def hamming_distance(string)
    small, large = [strand, string].sort_by(&:size)
    mutations = 0
    small.split(//).each_with_index do |point, idx|
      mutation += 1 if point != large[idx]
    end
    mutations
  end
end
